...
- EH21 has now been tested and validated with a protected automation-supervision workspace, durable lease plus retry metadata, stale-intent invalidation, bounded Telegram delivery retries, and dead-letter visibility for the current approval-based automation baseline.
...
- EH16.1 implementation notes for future developers:
  ...
  3. `src/features/automation/lib/openclaw-adapter.ts` remains the protected planner boundary, but EH19 now routes that seam through a private OpenClaw gateway when configured and still returns `null` on provider failure so the app can fall back to deterministic local triage safely.
...
- Status: `validated locally, hold before EH19`
- Notes: EH18 is now implemented on top of the validated EH17 approval foundation. Prisma now persists `TelegramStaffIdentity` and `TelegramCashierSession`, so Telegram-originated cashier identity, bounded conversation state, and payment outcome linkage no longer have to live in memory or inside approval payloads alone. `/api/automation/telegram` now accepts inbound Telegram webhook messages for linked cashier accounts, keeps the existing approval callback path, and runs a deterministic cashier assistant that can parse payer-plus-amount text, narrow to one exact open bill, ask numbered clarification questions, require explicit partial-payment confirmation, and require explicit cash-in-hand confirmation before a `PAYMENT_POST` approval request is sent. Approved `PAYMENT_POST` intents now execute through the existing DWDS payment workflow and return the receipt outcome, while `/admin/payments` now exposes both cashier-owned Telegram identity linking and a visible Telegram-origin audit panel for recent sessions, approval state, and resulting receipts. Local validation has now covered Prisma client regeneration, `tsc --noEmit`, `eslint`, successful migration deployment on the local PostgreSQL database, end-to-end approval execution for a full cash settlement on `DWDS-SMP-1001`, receipt creation, bill resolution, and successful name-based payer parsing with `pay Mario Ramos 456.06`. Live webhook delivery is confirmed, but full live end-to-end validation remains blocked by the lack of open-bill test data on the deployed environment. EH19 must remain blocked until the user explicitly authorizes moving on.
- Continuation note: the next unresolved production issue is `/admin/meters` on the live site. Vercel logs showed `public.MeterReplacementHistory` missing in production, so the table was manually created in Supabase SQL Editor without foreign keys after the direct Prisma migration path failed from the local machine because the Supabase direct host on `:5432` was unreachable. The next session should first reload `/admin/meters`; if it still fails, use the new Vercel runtime error to identify the next missing production table, column, or relation and repair that dependency iteratively. This production cleanup is separate from EH18 and must not be treated as authorization to start EH19.

### EH19: OpenClaw Integration
- Status: `validated`
- Notes: EH19 now replaces the old stub-only planner seam with a private OpenClaw gateway client in `src/features/automation/lib/openclaw-gateway.ts` plus a bounded adapter in `src/features/automation/lib/openclaw-adapter.ts`. Follow-up triage and exception summarization can now accept OpenClaw-ranked structured proposals first, and the Telegram cashier assistant can now use the same planner boundary for payment-detail extraction, numbered bill-choice interpretation, partial-payment confirmation, and cash-received confirmation. The validated live path now uses the dedicated OpenClaw `dwds` agent through the gateway `responses` endpoint, stores visible provider or model metadata on worker runs, surfaces fallback reasons when OpenClaw degrades, and normalizes ranking-only planner output into bounded DWDS proposals. DWDS still performs all bill matching, approval persistence, payment posting, receipts, and audit logging, and every OpenClaw-backed path still degrades safely to the existing deterministic baseline when the gateway is missing, unavailable, or returns invalid structured output.
